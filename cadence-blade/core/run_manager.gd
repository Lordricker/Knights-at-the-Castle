...
#   red_knight_scene, green_archer_scene, and rogue_scene in the Inspector.
...
@export_group("Players (Online — wire all)")
...
## PackedScene for the Blue Rogue character (used in multiplayer spawning).
...
func _spawn_players() -> void:
	...
	if remembered != "":
		...
		if scene_map.has(remembered):
			...
			var key_order: Array[String] = ["red_knight", "green_archer", "rogue"]
	...
	var key_order_default: Array[String] = ["red_knight", "green_archer", "rogue"]
	for i in player_scenes.size():
...
func _char_scene_map() -> Dictionary:
	...
	if rogue_scene != null:        d["rogue"]        = rogue_scene
	return d
