...
## Returns true while the current attack is still in its windup (pre-pause-frame) phase.
func _is_windup_state() -> bool:
	return attack_state == AttackState.SHOOT_WINDUP or attack_state == AttackState.PIERCE_WINDUP


## Rotates the aim pointer with up/down input, or towards the mouse cursor once the player
## moves it during windup. Called every physics frame.
func _update_aim_pointer(delta: float) -> void:
	...
	if _aim_mouse_active:
		...
		target_deg = clampf(
				rad_to_deg(atan2(to_mouse.y, to_mouse.x * facing)),
		...
		target_deg = clampf(
				...
				aim_angle_min_deg, aim_angle_max_deg)
	# Same max angular speed either way, so mouse-aim eases in instead of snapping.
	_aim_angle_deg = move_toward(_aim_angle_deg, target_deg, aim_degrees_per_second * delta)
